...
const err = {
  ...
  'Signup requires a valid password': 'Contraseña débil (mín. 6)',
  'Email address is invalid': 'Email no válido',
  'Too many requests': 'Demasiados intentos. Espera unos minutos.'
};

const getErrMsg = (e) => {
  const msg = e?.message || '';
  return err[msg] || msg || 'Ha ocurrido un error inesperado';
};
...
const entrar = wi => {
  ...
  supabase.from('smiles')
    ...
    .eq('id', wi.id)
    .then(({ error }) => { if (error) console.error('Error actualizando actividad:', error); });
    ...
    if (btnCG) {
      ...
      try {
        await accion(btnCG, 'Finalizando', async () => {
          const partes = user.user_metadata?.full_name ? user.user_metadata.full_name.split(' ') : ['Usuario', ''];
          const wi = {
            id: user.id,
            usuario: u,
            email: user.email,
            nombre: user.user_metadata?.given_name || partes[0],
            apellidos: user.user_metadata?.family_name || partes.slice(1).join(' ') || '',
            rol: rolPublico,
            activo: true,
            estado: 'activo',
            terminos: true,
    ...
    if (btnLogin) {
      ...
      await accion(btnLogin, 'Iniciando', async () => {
        ...
        if (!input.includes('@')) {
          ...
          if (rpcErr || !resolvedEmail) throw new Error('Usuario no encontrado');
          email = resolvedEmail;
        }
        ...
        // Recuperar perfil de la tabla smiles
        ...
        if (profError || !profile) throw new Error('Perfil no encontrado en la base de datos.');
    ...
    if (btnRec) {
      ...
      await accion(btnRec, 'Enviando', async () => {
        ...
        if (!emailVal.includes('@')) {
          const { data: resolvedEmail, error: rpcErr } = await supabase.rpc('obtener_email_por_usuario', { username_buscado: emailVal });
          if (rpcErr || !resolvedEmail) throw new Error('Usuario no encontrado');
          email = resolvedEmail;
        }
