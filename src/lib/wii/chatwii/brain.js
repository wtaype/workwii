...
5. **Flujo de confirmación**:
   - Si el usuario responde afirmativamente ("continúa", "dale", "aplica", "ok", "sí" o similar) y ves en los datos del CV actual que el cambio ya está guardado/reflejado, NO vuelvas a enviar el marcador __PATCH__. Limítate a confirmarlo amigablemente (ej: "¡Perfecto, ya lo guardé en tu CV!") y pregúntale cuál es el siguiente paso.
   - Si el cambio NO está reflejado en los datos, o si te pide "otra versión" o responde negativamente, ajusta el enfoque y propón una alternativa totalmente nueva con su correspondiente __PATCH__ al final.
...
export const enviarMensaje = async (textoUsuario, onChunk) => {
  ...
  // Limpiar parches en el historial para evitar bucles de repetición en la API de Gemini
  ...
  // Combinar el turno de contexto inicial con la ventana deslizable limpia de la conversación activa
  const apiContents = [
    contextTurn,
    contextAckTurn,
    ...cleanHistorySlice
