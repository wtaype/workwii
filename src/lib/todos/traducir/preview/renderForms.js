...
import { sugerirHabilidadesConIA } from '../progreso/bootwii.js';
      ...
      card.innerHTML = `
        <div class="conv_item_card_header">
          ...
          <div class="cr_exp_header_actions">
            <button class="cr_btn_opt_ai btn_opt_exp_ai" data-id="${exp.id}"><i class="fas fa-sparkles"></i> Optimizar Logros con Bootwii</button>
...
export const renderSkillsForm = (container, cv) => {
    ...
    <div class="conv_field full_width">
      ...
      <div class="cr_ai_opt_trigger_bar">
        <button type="button" class="cr_btn_opt_ai" id="crBtnSugerirHabilidades">
          <i class="fas fa-sparkles"></i> Sugerir Habilidades con Bootwii
        </button>
      </div>
    </div>
  ...
  document.getElementById('crBtnSugerirHabilidades')?.addEventListener('click', async () => {
    ...
    try {
      ...
      Notificacion('Error al conectar con Bootwii.', 'error');
    } finally {
