/**
 * escribirMd.js - Parser de Markdown a HTML (la manito de Chatwii)
 * Traduce respuestas Markdown en elementos interactivos con contenteditable.
 * Escrito en espanol sin tildes y con cero dependencias para maxima portabilidad.
 */
...
/**
 * Sanitiza HTML basico contra XSS
 */
...
/**
 * Parsea un string de Markdown y devuelve HTML enriquecido e interactivo.
 */
export const mdToHtml = (md) => {
  ...
  const flushQuote = () => {
    ...
    if (alertMatch) {
      ...
      else if (type === 'CAUTION') { icon = 'fas fa-ban'; title = 'Precaucion'; }
  ...
  lines.forEach(line => {
    ...
    if (cbMatch) {
      ...
      // Renderiza bloque con contenteditable y boton de copiar integrado de forma portable
      result.push(`
        <div class="chatwii-codeblock-container">
          <div class="chatwii-codeblock-header">
            ...
            <button class="chatwii-codeblock-copy" onclick="navigator.clipboard.writeText(document.getElementById('${uniqueId}').innerText); if(window.Mensaje){window.Mensaje('Codigo copiado', 'success');}else{alert('Codigo copiado');}">
    ...
    if (listMatch) {
      ...
      result.push(`<li>${formatInline(text)}</li>`);
      return;
    } else if (inList) {
      result.push('</ul>');
      inList = false;
